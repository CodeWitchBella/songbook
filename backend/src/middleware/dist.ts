...
const distMiddleware = () => (req: express.Request, res: express.Response, next: express.NextFunction) => {
  if(settings.serveStatic) {
    staticMiddleware(req, res, next)
  } else {
    getFrontendFile(req.path.replace(/^\/dist/, ''), false, null)
      .then(f => {
        if(!f) {
          ...
          if(type) {
            if(/^text/.exec(type)) res.append('Content-Type', type+';charset=UTF-8')
