...
const all = merge([major, minor, sixth, seventh, ninth, minor7, maj7, dim, plus, sus]);
...
const allChordsSet = new Set(
  [all.czech, all.inter].reduce(flatReducer, []).filter((item, index, arr) => arr.indexOf(item) === index),
);
...
function chordsForLang(src: ChordDefSrc, _: "czech" | "inter") {
  return src.map(note =>
    typeof note === "string" ? [note] : note.map(chord => (typeof chord === "string" ? chord : chord.czech)),
  );
}
